...
class MedirectPipeline:

    # Database setup
    conn = sqlite3.connect('medirect.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `medirect`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM medirect WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO medirect (date, title, link, content)"
